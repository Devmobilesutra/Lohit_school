...
/**
 * Created by Ganesh Borse on 15/04/2017.
 */
public class TABLE_EXAMINATION_ATTENDANCE {
    private static String LOG_TAG = TABLE_EXAMINATION_ATTENDANCE.class.getSimpleName();
    public static String NAME = "tbl_examination_attendance";
    ...
    public static void save_student_examination_attendance(String teacher_id, String school_id, String standard_id, String boys_present_count,
        ...
        cv.put(COL_TERM, term.trim());
        cv.put(COL_SUBJECT, subject.trim());
    ...
    public static ArrayList<DTOStudentAttendance> getUnSyncedAttendance() {
        ...
        if (c.getCount() > 0) {
            ...
            do {
                ...
                String term = c.getString(c.getColumnIndexOrThrow(COL_TERM));
                String subject = c.getString(c.getColumnIndexOrThrow(COL_SUBJECT));

                DTOStudentAttendance studentAttendance = new DTOStudentAttendance(teacher_id, school_id, standard_id, boys_count,
                        girls_count, total_count, image_url, date, time, latitude, longitude, term, subject);
    ...
    public static void change_synced_flag(String teacher_id, String school_id, String standard_id, String attendance_date,
        ...
        try {
            long updated_count = db.update(NAME, cv, COL_TEACHER_ID + "=? AND " + COL_SCHOOL_ID + "=? AND " + COL_DATE + "=? AND "
                            + COL_STANDARD_ID + "=? AND " + COL_TERM + "=? AND " + COL_SUBJECT + "=?",
                    new String[]{teacher_id, school_id, attendance_date, standard_id, term, subject});
    ...
    public static ArrayList<LinkedHashMap<String,String>> getSmsAttendance() {
        ...
        if (c.getCount() > 0) {
            ...
            do {
                ...
                message += school_id + "&" + teacher_id + "&" + standard_id + "&" + term + "&" + subject + "&" + time
                        + "&" + boys_count + "&" + girls_count + "&" + total_count;

                /*message += school_id + "&" + teacher_id + "&" + standard_id + "&" + term + "&"  + subject + "&" + date
                        + "&" + time + "&" + boys_count + "&" + girls_count + "&"  + total_count + "&" + latitude + "&" + longitude ;*/
